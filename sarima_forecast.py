...
Forecasts "Average T.F. Heater Serpentine Coil Inlet Temperature" using SARIMA method.
Saves plots and metrics (MSE, RMSE, MAE) in SARIMA folder.
"""
...
try:
    ...
    print(f"   Model summary: AIC = {sarima_model_fit.aic:.2f}, BIC = {sarima_model_fit.bic:.2f}")
...
y_train_pred = sarima_model_fit.fittedvalues
# SARIMA fittedvalues might have different index due to differencing
# Align with full training set
y_train_pred_aligned = pd.Series(index=y_train.index, dtype=float)

# The fitted values start after differencing (d + D parameters)
start_idx = ARIMA_ORDER[1] + SEASONAL_ORDER[1] * SEASONAL_ORDER[3]  # d + D*s
...
with open(metrics_file, 'w') as f:
    f.write("SARIMA Forecasting Metrics\n")
    f.write("=" * 40 + "\n\n")
    ...
    f.write(f"Model AIC: {sarima_model_fit.aic:.4f}\n")
    f.write(f"Model BIC: {sarima_model_fit.bic:.4f}\n\n")
...
sns.lineplot(x=dates_test, y=y_pred, 
             ax=ax, color='darkorange', label='Forecasted (SARIMA)', linewidth=2)

ax.set_title(f'SARIMA Forecasting: {TARGET_COLUMN}\nOrder (p,d,q)(P,D,Q,s) = {ARIMA_ORDER}{SEASONAL_ORDER} | Test MSE: {test_mse:.2f}, RMSE: {test_rmse:.2f}, MAE: {test_mae:.2f}', 
...
sns.lineplot(x=dates_test, y=y_pred, 
             ax=ax, color='darkorange', label='Forecasted (SARIMA)', linewidth=2.5, marker='s', markersize=4)

ax.set_title(f'SARIMA Forecasting - Test Period Zoom\nOrder (p,d,q)(P,D,Q,s) = {ARIMA_ORDER}{SEASONAL_ORDER} | Test MSE: {test_mse:.2f}, RMSE: {test_rmse:.2f}, MAE: {test_mae:.2f}', 
             fontsize=14, fontweight='bold')
ax.set_xlabel('Date Time', fontsize=12)
ax.set_ylabel('Temperature', fontsize=12)
ax.legend(fontsize=10, loc='best')

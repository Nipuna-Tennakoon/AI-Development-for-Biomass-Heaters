"""
Auto-Regressive (AR) Forecasting Script
Forecasts "Average T.F. Heater Serpentine Coil Inlet Temperature" using AR method.
Saves plots and metrics (MSE, RMSE, MAE) in AR folder.
"""
...
LAG_ORDER = 5  # AR order (p parameter)
...
print("=" * 60)
print("Auto-Regressive (AR) Forecasting")
print("=" * 60)
...
# AR Forecasting
print("\n3. Performing AR forecasting...")
print(f"   Lag order (p): {LAG_ORDER}")

# Fit AR model on training data
print("   Fitting AR model on training data...")
try:
    ar_model = AutoReg(y_train, lags=LAG_ORDER)
    ar_model_fit = ar_model.fit()
    print(f"   Model fitted successfully")
    print(f"   Model summary: AIC = {ar_model_fit.aic:.2f}")
...
y_train_pred = ar_model_fit.fittedvalues
# AutoReg fittedvalues start from index LAG_ORDER, so align with full training set
# Create aligned series with same index as y_train
y_train_pred_aligned = pd.Series(index=y_train.index, dtype=float)
start_idx = LAG_ORDER

# Assign fitted values (they start from position LAG_ORDER)
if len(y_train_pred) > 0:
    ...
    # Fill the first LAG_ORDER values with the first available prediction
...
y_pred = ar_model_fit.forecast(steps=len(y_test))
...
sns.lineplot(x=dates_test, y=y_pred, 
             ax=ax, color='darkorange', label='Forecasted (AR)', linewidth=2)

ax.set_title(f'AR Forecasting: {TARGET_COLUMN}\nLag Order (p) = {LAG_ORDER} | Test MSE: {test_mse:.2f}, RMSE: {test_rmse:.2f}, MAE: {test_mae:.2f}', 
...
sns.lineplot(x=dates_test, y=y_pred, 
             ax=ax, color='darkorange', label='Forecasted (AR)', linewidth=2.5, marker='s', markersize=4)

ax.set_title(f'AR Forecasting - Test Period Zoom\nLag Order (p) = {LAG_ORDER} | Test MSE: {test_mse:.2f}, RMSE: {test_rmse:.2f}, MAE: {test_mae:.2f}', 
             fontsize=14, fontweight='bold')
ax.set_xlabel('Date Time', fontsize=12)
ax.set_ylabel('Temperature', fontsize=12)
ax.legend(fontsize=10, loc='best')

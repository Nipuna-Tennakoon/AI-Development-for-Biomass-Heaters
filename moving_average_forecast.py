"""
Moving Average Forecasting Script
Forecasts "Average T.F. Heater Serpentine Coil Inlet Temperature" using Moving Average method.
Saves plots and metrics (MSE, RMSE, MAE) in Moving average folder.
"""
...
print("\n3. Performing Moving Average forecasting...")
print(f"   Window size: {WINDOW_SIZE}")

# Calculate training predictions (using rolling window on training data)
y_train_pred = y_train.rolling(window=WINDOW_SIZE).mean()
# Handle NaN values at the beginning
y_train_pred = y_train_pred.bfill().ffill()

# Forecast on test set using moving average
y_pred = pd.Series(np.zeros(len(y_test)), index=y_test.index)

for i in range(len(y_pred)):
    # Combine training data with previous predictions
    combined_series = pd.concat([y_train, y_pred.iloc[:i]]).reset_index(drop=True)
    # Calculate moving average
    y_pred.iloc[i] = combined_series.rolling(window=WINDOW_SIZE).mean().iloc[-1]

# Handle NaN values (if window_size is larger than available data at the start)
y_pred = y_pred.bfill().ffill()
...
with open(metrics_file, 'w') as f:
    f.write("Moving Average Forecasting Metrics\n")
    f.write("=" * 40 + "\n\n")
...
sns.lineplot(x=dates_test, y=y_pred, 
             ax=ax, color='darkorange', label='Forecasted (Moving Average)', linewidth=2)

ax.set_title(f'Moving Average Forecasting: {TARGET_COLUMN}\nWindow Size = {WINDOW_SIZE} | Test MSE: {test_mse:.2f}, RMSE: {test_rmse:.2f}, MAE: {test_mae:.2f}', 
...
sns.lineplot(x=dates_test, y=y_pred, 
             ax=ax, color='darkorange', label='Forecasted (Moving Average)', linewidth=2.5, marker='s', markersize=4)

ax.set_title(f'Moving Average Forecasting - Test Period Zoom\nWindow Size = {WINDOW_SIZE} | Test MSE: {test_mse:.2f}, RMSE: {test_rmse:.2f}, MAE: {test_mae:.2f}', 
             fontsize=14, fontweight='bold')
ax.set_xlabel('Date Time', fontsize=12)
ax.set_ylabel('Temperature', fontsize=12)
ax.legend(fontsize=10, loc='best')
